...
(function(){
  ...
  window.openReader = async function(title,ch){
    ...
    if(!window.accountSession) return;
    const {data,error}=await getSB().from('chapters').select('id,chapter_number,title_id,titles!inner(title)').eq('chapter_number',ch).eq('titles.title',title).maybeSingle();
    if(!error && data){ window.currentDbChapterId=data.id; await saveReadingProgress(data.id,1); }
  };
